...
class ToolRequest(BaseModel):
    ...
    text: str | None = Field(
        default=None,
        description=(
            "Character's reply text. Required when action='respond'. "
            "Wrap in-character actions with *asterisks*, e.g. '*웃으며 고개를 끄덕인다* 안녕하세요!'"
        ),
    )
...
def build_agent(
    ...
    system = persona_system_prompt if persona_system_prompt else SYSTEM_PROMPT
...
def plan_with_gemma(
    user_request: str,
    workspace: Path,
    agent: Agent[None, ToolRequest] | None = None,
    memory_context: str = "",
) -> RunSnapshot:
    ...
    try:
        ...
        result = agent.run_sync(_wrap_request(user_request, workspace, memory_context))
